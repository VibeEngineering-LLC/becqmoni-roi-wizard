...
STRINGS = [
    ...
    ("tipConfigName",
     "The name becomes the file name under config\\ROI; a matching name overwrites that file.",
     "Имя становится именем файла в config\\ROI; совпадение имён перезапишет этот файл."),
    ("tipSetName",
     "Name of the set in the nuclide library; a duplicate name is confirmed before saving.",
     "Имя набора в библиотеке нуклидов; повторное имя приложение переспросит перед записью."),
    ("tipFullSet",
     "Applies to the SET only: the ROI configuration is always built from the table.",
     "Относится только к НАБОРУ: ROI-конфигурация всегда строится по таблице."),
    ("tipAnchorManual",
     "Anchor chosen by hand. Disabled with the full set: there the anchors are picked automatically.",
     "Якорь, выбранный руками. При полном наборе недоступен: там якоря выбираются автоматически."),
    ("tipAnchorCount",
     "How many lines to mark as anchors when they are picked automatically.",
     "Сколько линий помечать якорями при автоматическом выборе."),
    ("statusRoiNotMeasurable",
     "{0} of them do not measure area (markers: no zone, no Bq/cps coefficient)",
     "из них не измеряют площадь: {0} (маркеры: зоны нет, коэффициент Бк/имп не задан)"),
    ("issueMixedChains",
     "the set mixes decay series ({0}): the anchor gate is common to the whole set, "
     "so one matched anchor switches the others on as well and they yield false identifications",
     "в наборе смешаны цепочки распада ({0}): якорный гейт общий на весь набор — "
     "одного совпавшего якоря достаточно, чтобы включились и остальные, "
     "а они дают ложные отождествления"),
    ("issueAnchorIsXrf",
