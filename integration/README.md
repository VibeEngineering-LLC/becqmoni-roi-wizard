...
успехе, 1 при провале, так что скрипт годится и для CI. `SetExporter.cs` и файлы формы в
эту сборку не входят: они зависят от типов хоста, а проверяется расчётная часть.
...
На момент последнего прогона: **52 проверки, все зелёные**.
...
- **Якорем не может быть ХРИ или вторичный маркер.** У первых интенсивность условная
  ...
  опору. Порог 0,2·max тоже считается по одним γ-линиям.
