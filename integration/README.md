...
- **Хотя бы один `IsAnchor = true` на набор.** Без якоря `LibraryPeakFitter` не стартует
  вовсе. Автовыбор — `AnchorPicker.Pick`: сильная и одинокая линия (для ряда Th-232 это
  2614,5 кэВ, для Ra-226 — 609,3). Модуль ставит ровно один якорь; сам `LibraryPeakFitter`
  при этом перебирает **все** линии с `IsAnchor`, берёт сдвиг калибровки с сильнейшей по
  SNR и требует совпадения хотя бы одной с найденным пиком (допуск 0,5·FWHM). То есть
  единственный якорь — единственная точка отказа: не нашёлся 2614,5 — набор молчит.
  Помечать 2–3 сильные и одинокие линии — задача следующей итерации.
