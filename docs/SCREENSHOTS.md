...
Заголовок окна — тёмно-синий, тем же тоном `--head`, которым окрашены заголовки
панелей темы; текст белый, пиктограммы приложения в заголовке нет, скругление
углов уменьшенное (`DWMWCP_ROUNDSMALL`). Всё это ставит `WizardTheme.ApplyCaption`
через DWM API — на Windows до 11 22H2 атрибуты тихо игнорируются, и окно остаётся
со стандартным заголовком. В правом углу ряда вкладок — булавка «поверх остальных
окон»: в системный заголовок Windows свою кнопку не пускает, поэтому она живёт
на ближайшем к нему месте, как закрепка у док-панелей самого BecqMoni.

Снимок сделан из живого приложения: свежий клон `master`, `apply_patch.py`,
сборка Release, пункт меню «Инструменты» (русская локаль).

![Окно с оформлением BecqMoni](screenshots/14-window-chrome.png)
