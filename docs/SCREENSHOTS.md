...
Справка описывает МОДУЛЬ и ведётся отдельно от текста страницы: ресурс `help.xml`
собирается генератором `integration/tools/gen_help.py`. Раньше он выгружался из
`index.html` — чтобы две версии не разъехались, — но после переработки под WinForms они
разошлись всё равно, и общий источник этого не показал: выгрузку просто перестали
запускать. Окно справки — такая же
