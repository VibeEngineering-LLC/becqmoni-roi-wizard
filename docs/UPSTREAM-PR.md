...
- [x] **Предупреждения сборки — не от вклада** (проверено 26.07 контрольным опытом).
      Чистое дерево `master` без модуля, открытое в VS 2022 и собранное, даёт те же
      четыре предупреждения: дважды `MSB3327` (нет сертификата подписи манифеста
      ClickOnce), «The package Microsoft.Windows.SDK.Contracts … could not be found …
      Run a NuGet package restore» и `GetAssembliesMetadata` с `ArgumentException` —
      последнее следствие первого, задача читает `.winmd` из невосстановленного пакета.
      С модулем набор тот же. Единственное предупреждение, которое давал сам модуль
      (`CS0108`: метод `Tag` в `HelpForm` скрывал `Control.Tag`), устранено —
      метод переименован в `ApplyTag`.
...
### Что выяснено про встроенную базу (26.07, разбор по исходникам)

Проверялось из-за вопроса «нет ли архитектурного противоречия: у BecqMoni база
подключена к API IAEA, а у модуля своя фиксированная».

**Встроенная база к API не подключена.** `nucdb.sqlite` (5,6 МБ) лежит в дереве как
`<Content>` (`BecquerelMonitor.csproj:1233`) и открывается **только на чтение**:
`Data Source=<папка приложения>\nucdb.sqlite; Mode=ReadOnly` (`NucBase/DataBase.cs:18`).
Сетевых вызовов во всём проекте нет — ни `HttpClient`, ни `WebClient`, ни `WebRequest`,
ни `Download*`; единственные URL — ссылки «открыть в браузере» в `AboutForm` и `MainForm`.
То есть у приложения тоже **снимок**, просто больший и в другом формате.

Содержимое (прямой слепок ENSDF/IAEA): `nuclides` 4377, `decay_radiations` 66 290
(из них 65 906 с энергией), `decay_chain` 4101, `l_decays` 5931, `cumulative_fission`
6326, `thermal_cross_sect` 502. γ-линии настоящие: Cs-137 → 661.657 при 85.1 %.
Используется база только окном-справочником (`MainForm.ShowNucBaseView`) и поиском по
энергии из окна обнаружения пиков (`CallNucBaseSearch`) — в расчёты она не входит.

**Значит расхождение не «живое против замороженного», а дублирование:** два независимых
снимка одних и тех же первичных данных в одном приложении, обновляемые разными
процедурами. Чего в `nucdb` нет и что несёт наш ресурс: коды семейств ANSI N42.34,
пресеты, каталог ХРИ материалов защиты и детектора (рентген там привязан к
родителю-распаду, а свинец защиты ничего не распадает), коэффициенты равновесия рядов,
подписи RU/EN. Технические шероховатости при переходе: `energy` — строка и иногда
диапазон («36.304 - 36.660»), интенсивности нужно приводить к нашей нормировке, чтение
требует, чтобы рабочей папкой была папка приложения (сейчас модуль тестируется автономно).

**Возможное решение — гибрид:** энергии и интенсивности γ/X брать из `nucdb`, когда он
доступен, а свой ресурс оставить слоем курирования (семейства, ряды, ХРИ, пресеты,
подписи), с запасным путём для автономной работы. **Решение отложено до ответа автора** —
это его архитектурный выбор, а не наш.
