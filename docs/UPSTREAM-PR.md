...
**Состояние на 25.07.2026:** решение с модулем собирается (`BecquerelMonitor.exe`, ноль
ошибок), тесты модуля — 100 проверок, все зелёные, прогон на реальном спектре сделан
(см. чек-лист). Из чек-листа открыт один пункт — открыть форму в дизайнере Visual Studio:
на машине, где велась работа, стоят только Build Tools.
...
4. **Якоря — главный вопрос.** Модуль помечает **несколько** якорных линий (по умолчанию
