...
3. **Обновление списков в интерфейсе.** Набор, записанный из мастера (как и из штатного
   `NuclideSetForm`), появляется в списке «Набор» окна обнаружения пиков только после
   перезапуска: у `ROIConfigManager` событие `ROIConfigListChanged` есть, у
   `NuclideDefinitionManager` события нет, а `DCPeakDetectionView.RefreshNuclideSets()`
   вызывается только из конструктора. Пробное решение (событие в менеджере + подписка
   вида) **откачено**: `RefreshNuclideSets()` заодно присваивает `SelectedIndex`, и
   вызов его в произвольный момент ломал поведение поиска пиков. Как правильно уведомлять
   интерфейс — вам виднее; сами мы в это больше не вмешиваемся.
